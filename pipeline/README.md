...
| `publish` | `grid/dist_<S>.tif`, `land.tif`, `frame.json`, `fetch/<name>.poly`, `poles/A.json`, `B.json`, `units.json`, `roads/`, `validate/report.json` and the two HTML reports | `inputs.json` (the stamp the explore artefacts were built from), `inside.tif`, `edgeband.tif`, `explore_<S>.tif`, `A.pmtiles`, `B.pmtiles`, `detail/<unit>/<scenario>-<lat>_<lon>.png` and `.json` (keyed by the pole's six-decimal coordinates, so a rerun never rewrites a key the live site reads) under the stamp `detail/published.json`, then the R2 objects and the site JSON | 962 s local part, the upload on top |
...
| `top_n` | int | how many mainland poles are kept per unit and scenario; island poles ranked above the last of them ride along, at most `top_n` of those, so a unit publishes between `top_n` and `2 * top_n` |
...
| `min_island_m2` | int | no pole on a land component smaller than this; a pole on a component that is not its unit's largest is published with its area. A component is 8-connected land on the coarse grid; its area is its land polygons clipped to its outline when its cell count is under 5,000 km2 (all-touched cells make an island of a reef), the cell count above; a small component whose land, less the big water, runs on into a bigger component is a raster fragment of that body and counts as it. The refinement also refuses any point whose body of land (its land polygon less the big water) is under the floor, which is the grid-independent reading of the same rule. Default 1000000, the same 1 km2 as the water-body threshold |
