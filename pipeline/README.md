...
| `publish` | `grid/dist_<S>.tif`, `land.tif`, `frame.json`, `fetch/<name>.poly`, `poles/A.json`, `B.json`, `units.json`, `roads/`, `validate/report.json` and the two HTML reports | `inside.tif`, `edgeband.tif`, `explore_<S>.tif`, `A.pmtiles`, `B.pmtiles`, `detail/<unit>/<scenario>-<rank>.png` and `.json`, then the R2 objects and the site JSON | 962 s local part, the upload on top |
