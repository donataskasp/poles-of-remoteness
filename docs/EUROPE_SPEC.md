...
- **Islands** (2026-09-05, #30). A candidate cell must lie on a land component of at least `min_island_m2` (default 1 km², the water-body threshold), where a component is 8-connected land on the coarse grid; a rock that rasterises to one all-touched cell is not a place. A pole whose component is not its unit's main one (the component holding most of the unit's candidate cells) is published with `island_km2`, the component's area, and the site can hide such poles. The definition is unit-relative on purpose: Iceland's mainland is not an island, Zealand, Crete and Baffin are. A strait narrower than one coarse cell joins an island to its neighbour. Before this rule sub-cell rocks legitimately won poles (Kolbeinsey, Bogskar, Palagruza).
