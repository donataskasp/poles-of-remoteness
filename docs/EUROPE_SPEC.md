...
| `territory_mask` | Svalbard, Jan Mayen, Franz Josef Land, Novaya Zemlya, Azores, Madeira, Rockall | Northwestern Hawaiian Islands (added 2026-08-23 after Laysan saturated the 500 km cap; the unit list is explicit, so the mask trims Hawaii rather than removing a unit) |
...
- **Islands** (2026-09-05, #30). A candidate cell must lie on a land component of at least `min_island_m2` (default 1 km², the water-body threshold), where a component is 8-connected land on the coarse grid and its area is its land polygons clipped to its outline for components under 5,000 km2 of cells (all-touched rasterisation makes a 1.2 km2 island of the 0.1 km2 of rocks at Les Minquiers) and the cell count above, a raster fragment whose land runs on into a bigger body counting as that body; and the refinement refuses any point whose body of land (its land polygon less the big water) is under the floor, so the floor does not depend on the grid; a rock that rasterises to one all-touched cell is not a place. A pole whose component is not its unit's main one (the component holding most of the unit's candidate cells) is published with `island_km2`, the component's area, and the site can hide such poles. The definition is unit-relative on purpose: Iceland's mainland is not an island, Zealand, Crete and Baffin are. A strait narrower than one coarse cell joins an island to its neighbour. Before this rule sub-cell rocks legitimately won poles (Kolbeinsey, Bogskar, Palagruza).
- **Distinct areas** (2026-09-05, #56). Two poles of a unit count as separate places only when every path between them over land drops below `area_col_fraction` (default 0.5) of the nearer pole's distance to a road: connectivity of the land-masked superlevel set of the coarse grid at that threshold, the threshold quantised down onto a ladder of 1 percent steps, which is part of the rule (down can only make it stricter). The `dedup_m` floor (default 10 km) stays underneath it for plateau ties. This replaces the fixed 10 km deduplication of the first runs, under which a large unit's ten poles tiled its single remotest plateau.
...
5. **poles**: per unit and scenario, branch-and-bound over the coarse grid: a cell is refined only if `coarse_value + 2 * half_diagonal * (1 + distortion_pad) >= best_confirmed_lower_bound`, where the pad is the LAEA scale error at that cell (2% at 2,500 km from centre). Refinement: exact vector distances (STRtree over the scenario's ways within `coarse_value * 1.2 + 1 km`) at 25 m then 5 m in the cell's UTM zone, as in the LT method. Poles are accepted greedily in descending distance under the `dedup_m` floor and the distinct-area rule (2.3), and the list is a superset: the search runs until it holds `top_n` mainland poles, taking along the island poles ranked above the `top_n`-th mainland pole, at most `top_n` of them, so a unit publishes between `top_n` and `2 * top_n` poles, each with its overall rank, `island_km2`, nearest way (id, highway, name, ref, country), nearest settlement (name, type, distance, coordinates), and unit membership. The site shows the top `top_n` with islands, or the `top_n` mainland poles without.
...
| poles | 834 s for 104 jobs (52 units x 2 scenarios) on 4 workers, including the unit rebuild; the road tiles are a one-off 49 min before it | 2.4 GB parent plus 6.6 GB cumulative across the 4 workers | 30.8 GB (road tiles 28.9, land index 1.36, water 0.47, countries and units 0.06; GB throughout, the road tiles are 26.87 GiB) |
...
| publish (local part) | 962 s: masks 9 s; per scenario quantise 25 s, warp to EPSG:3857 26 s (A) and 24 s (B), `gdal raster tile` 8 s and 9 s, MBTiles packing 199 s and 173 s, `pmtiles convert` 1 s; detail rasters 463 s for 909 rasters as 104 jobs on 4 workers | gdalwarp 2.61 GB; tiler and packer under 0.2 GB; detail pool 1.3 GB in total, 0.37 GB per worker | 0.98 GB (MBTiles 0.25, archives 0.24, tile directories 0.22, Mercator rasters 0.17, class rasters 0.07, masks and detail 0.02) |
...
Stage 3's local part adds 16 minutes (2026-08-22, commit 3b0dbd4). The class rasters hold 675.4 M cells: 525.6 M are NODATA (77.8 %: sea, plus the 9.9 M km2 of land the rectangular frame holds outside the source polygons, from the Sahara to Siberia), 3.8 M are the 50 km edge band and 146.0 M carry a class; A and B share the masks cell for cell. The MBTiles packer (Python, one tile at a time) is the slow step at about three minutes per scenario against eight seconds for the GDAL tile cut. Five of the 909 detail rasters are all NODATA because their poles sit on islets narrower than a 50 m pixel (Bell Rock for gb A rank 6 and B rank 7, Kolbeinsey for is A and B rank 1, a Pembrokeshire rock for gb B rank 4); they are published with a warning since the pole itself lies within 100 m of indexed land. The run stops, as designed, with `PublishError` naming the five R2 variables and leaves no `done.json`; the upload, the HEAD verification and the site documents follow the R2 enablement as a rerun of the same command.
...
North America (snapshot 2026-08-22, stage 5, run of record 2026-08-23, the same machine with colima stopped; one source PBF of 19.3 GB pre-downloaded and adopted by the fetch stage):

| Stage | Wall clock | Peak RSS | Disk after |
| --- | --- | --- | --- |
| fetch | 33 s verification of the pre-downloaded `north-america-latest.osm.pbf` (one source, no supplements) | 0.12 GB | 19.3 GB |
| extract | 1,273 s from scratch (boundaries 4,982, highways 61,336,848, places 239,493, water 14,028,586, land 870,948 features) | osmium and ogr2ogr children 5.2 GB cumulative | 40.6 GB |
| classify | 174 s | under 0.2 GB | 22.0 GB (roads_A 47,942,361 ways, roads_B 45,228,459) |
| grid | 799 s at the 500 km cap: rasterize A 86 s and B 75 s, EDT 234 s and 225 s as 120 tiles per scenario, land mask 149 s, A <= B check 29 s | gdal_rasterize 10.7 GB; EDT parent 11.3 GB plus 5.0 GB per worker | 11.4 GB |
| poles | 6,163 s for 128 jobs (64 units x 2 scenarios) on 2 workers, 197,848 refinements; the road tiles (214 tiles, about two hours) were built by the morning's first attempt and reused by every later one | 2.2 GB parent plus 6.6 GB cumulative across the 2 workers | 20.8 GB (road tiles 16.5, water 1.5, land index 1.3) |
| validate | 8,079 s: check 1 recheck 947 s, check 3 edge bound 256 s, check 4 grid shift 6,748 s, check 5 holes 27 s, checks 2, 6 and 7 under 2 s, contact sheet 101 s | 11.0 GB parent plus 11.1 GB cumulative across the workers | 1.73 GB (shifted grids, `contact-sheet.html` 13.6 MB) |
| publish (local part) | 2,569 s: masks 23 s; per scenario quantise 57 s and 56 s, warp to EPSG:3857 44 s and 42 s, `gdal raster tile` 19 s and 18 s, MBTiles packing 423 s and 391 s, `pmtiles convert` 2 s; detail rasters 1,492 s for 1,262 rasters as 128 jobs on 4 workers | gdalwarp 2.70 GB; the tiler 0.16 GB; the packer runs in the parent at about 1.2 GB; the detail pool about 1.3 GB in total by the watcher's samples | 1.8 GB (MBTiles 0.44, archives 0.40, tile directories 0.44, Mercator rasters 0.30, class rasters 0.14, masks and detail 0.02) |

The frame is 46,688 x 40,823 cells (1,906 M) at 250 m in an equal-area projection centred on 50 N 100 W, with the 500 km margin the cap needs; the EDT ran as 120 tiles per scenario with an overlap of 2,000 cells and no doubling; 1,094.6 M (A) and 1,106.5 M (B) cells saturate at 500 km; road cells 52.3 M (A) and 41.2 M (B); land cells inside the frame 459.7 M (28.7 M km2: the margin reaches beyond the extract into Central America, the Caribbean and Chukotka), of which the 64 units cover 18.8 M km2. The search: 64 units (the 50 states with the District of Columbia, the 10 provinces and 3 territories), 1,266 poles, no unit skipped; the slowest jobs were Nunavut B at 2,046 s and 40,761 refinements, Nunavut A at 1,554 s and 23,995, Yukon B at 1,499 s, and the first two would have died at the 20,000 cap of the Europe run (#46 raised it to 200,000 and tightened the bound). The search workers take 2 to 6.5 GB each on these units against Europe's 3.1 GB, so the run used two (`POLES_WORKERS=2`); an earlier attempt with four went into swap, and the culprit was a macOS daemon (`mediaanalysisd`, 27 GB compressed) rather than the workers, so `top` comes before blaming the pipeline. Validation: 0 blocking failures and 5 warnings; check 1 1,266 of 1,266 (all twenty Nunavut poles included), check 2 1,266 of 1,266, check 3 1,262 of 1,266 with 4 poles excluded at the data edge (Alaska ranks 9 and 10 in both scenarios, the western Aleutians measured against the extract's open-ocean southern boundary at 50 N: the Hornstrandir limitation again, #19), check 4 128 of 128, check 5 no hole warning in 384, check 6 Yellowstone reproduced (A 34,854.43 m against the published 34,854.6 m, 84 m apart) while the Frank Church reference (25,053.6 m, measured in 2017) failed non-blocking against our rank 5 at 19,289.49 m, 8.2 km away, a road mapped since; check 7 324 of 324 invariants and 64 of 64 expected units. Total disk under `work/north-america/2026-08-22/` about 110 GB. Against Europe: 2.8 times the cells, 7.4 times the search time (bigger units on half the workers) and 5.7 times the validation (check 4 re-searches every unit on the half-shifted grid); about 5.3 hours of compute end to end without the road tiles, all of it unattended.
...
- **R2** (public bucket, read-only to the world): `<region>/<snapshot>/A.pmtiles`, `B.pmtiles`, `detail/<code>/<scenario>-<rank>.png` with a `.json` georeference sidecar, `validation/` (report and contact sheet). Measured 2026-08-20 (stage 1, task 1.8) from the Lithuania 25 m grid quantised to the class table, downsampled to 250 m, tiled z0-z9 in EPSG:3857 as 8-bit grey PNG, packed with `pmtiles convert`: z9 tiles average 6.3 KB (A) and 6.5 KB (B), at most about 20 KB; z0-z8 add 0.57x the z9 bytes; the archive costs 15.2 (A) and 15.9 (B) bytes per km2 of land. Measured 2026-08-22 (stage 3, Europe snapshot 2026-08-19, pipeline commit 3b0dbd4; the z0-z9 pyramid from `gdal raster tile` with mode overviews, blank tiles dropped): `A.pmtiles` 114,160,033 bytes and `B.pmtiles` 128,332,120 bytes (8,137 and 8,138 tiles, 5,799 of them at z9, 18 and 17 blank tiles skipped), which is 12.2 and 13.7 bytes per km2 of the 9.36 M km2 of land inside the source polygons (149.8 M cells; the 10.2 M km2 assumption was close, and the per-km2 cost came in under the Lithuania projection because most of Europe sits in the coarser class bands). The 909 detail rasters with their sidecars total 13,583,553 bytes (14.9 KB per raster); the three validation artefacts 12,501,121 bytes. One Europe snapshot is 268.6 MB in R2 (256 MB without the validation files). Measured in the bucket 2026-08-23 after both regions were uploaded (`poles-data`, listed over S3): Europe 1,823 objects and 268,576,827 bytes, North America 2,529 objects and 436,256,747 bytes, 4,352 objects and 704,833,574 bytes in all, 6.6 percent of the free 10 GB; a full upload of both regions is about 4,500 Class A operations against the free million a month, and a rerun only HEADs (Class B). Measured 2026-08-23 (stage 5, North America snapshot 2026-08-22): `A.pmtiles` 206,704,617 bytes and `B.pmtiles` 197,608,566 bytes (26,413 and 26,201 tile entries, z0 to z9), 9.2 and 8.8 bytes per km2 of the 22.5 M km2 of land inside the source polygon (360.6 M classed cells); the 1,262 detail rasters with their sidecars total 16,205,121 bytes (12.8 KB per raster); the three validation artefacts 15,738,443 bytes. One North America snapshot is 436.3 MB in R2 and the two regions together 704.9 MB, against the 300 MB plus 335 MB of archives the plan had projected on 24.5 M km2. Two snapshots retained per region stays well inside the free 10 GB. Lithuania lies entirely in the fine 50 m and 100 m class bands, so the figure is conservative for remote terrain. Tooling notes for stage 3: `gdal raster tile --no-alpha` writes true single-band grey tiles (the MBTiles driver path adds an alpha band, 27% larger), and `gdaladdo` cannot build MBTiles overviews below z5, so the z0-z9 pyramid comes from `gdal raster tile`.
...
7. **Invariants**: A ≤ B at every published pole and every grid cell; every unit has `top_n` mainland poles or a documented reason, at most `top_n` island poles, and contiguous ranks; poles within a unit are at least `dedup_m` apart and no two of them are joined over land at `area_col_fraction` of the nearer one's distance (a consistency check through the same module the search uses, not an independent measurement); the unit count equals the region config's expectation; the class table is monotonic; every object the manifest references answers a HEAD request on R2 after upload; published JSON validates against its schema.
