...
**Fix round 1 (2026-08-23), the design as landed.** The M49 route does not work in browsers: Playwright's Chromium 151 returns `150` and `003` unchanged from `Intl.DisplayNames` while still giving "Lietuva" for `LT`; only node's ICU localises the area codes. The `code` key is gone. Each region config declares `names` by language (`{lt: Europa}`, `{lt: Šiaurės Amerika}`) with `name` as the English value and the fallback; `regions.json` publishes the map; `regionLabel(region, lang)` reads `names[lang] || name || id`; `regionName` is back to alpha-2 only. The loader rejects a missing or malformed `names` by name. DECISIONS 2026-08-23 has the entry; spec 5 now says country names come from `Intl.DisplayNames` and region names from the config.
