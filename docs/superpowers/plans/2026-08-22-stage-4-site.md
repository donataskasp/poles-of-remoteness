...
- Tests: `node --test dev/tests/` must stay green; TDD for every pure module (router, data, readout, palette, i18n, worker helpers, dev server range handling). UI modules are verified by screenshots.
...
Run: `cd /Users/donatas.kasparavicius/Personal/pole-of-remoteness && node --test dev/tests/`
...
`node --test dev/tests/` runs the pure-logic suites (router, data, readout, palette, i18n, worker helpers, the dev server). CI runs the same command (`site-tests.yml`).
...
Run `node --test dev/tests/` and make the expectations match what the existing functions return (read them; if `browserFamily('curl/8.4.0')` returns something other than `'Bot'`, use the value it returns for curl: the test pins today's behaviour, it does not change it).
...
`ranking.js` imports `i18n.js` only, so it loads in Node. Run `node --test dev/tests/`: green.
...
Run `node --test dev/tests/worker.test.mjs`: the two new tests fail with "landing is not a function" (or an import error).
...
Run `node --test dev/tests/`: green (the three pinned tests from Task 2 still pass because the helper bodies are unchanged).
...
The `html[lang="en"] [data-lang="lt"]` / `html[lang="lt"] [data-lang="en"]` rules from Task 1 hide the other language; the language toggle needs no extra code for the dialog.
