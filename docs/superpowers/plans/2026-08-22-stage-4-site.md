...
Start the dev server (Task 1 command) and, in a scratch directory with Playwright installed (`cd "$(mktemp -d)" && npm init -y >/dev/null && npm install playwright@1.62.1 >/dev/null`; the chromium build is in the cache, if Playwright asks for another run `npx playwright install chromium`), take one desktop screenshot to the scratchpad:
