...
- Produces:
  ...
  - `explore.js`: `createExploreLayer({url, palette, onReady}) -> Promise<layer>` where `layer` is a Leaflet `GridLayer` with `maxNativeZoom: 9`, `options.minZoom` from the archive header, `layer.classAt(latlng) -> number | undefined` (`undefined` while the covering tile is not loaded; `NODATA` for a tile the archive does not have), `layer.setPalette(palette)` repainting loaded tiles, `layer.header` (the PMTiles header).
