...
The `/data/europe/units.json` entry is the only region-specific line and it is data about the run of record, not behaviour: until `site/data` has the new JSON (after the R2 rerun), that URL returns the SPA page instead (about 4 KB), which still counts; note this in the commit message. The list is a contract: Task 8 (About, card) and Task 9 must not add a first-screen fetch without adding it here.
