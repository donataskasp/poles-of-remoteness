...
- Produces:
  ...
  - `data.js`: `getJSON(url, fetchFn = fetch)` cached by URL (failed loads are evicted so a retry refetches), `loadRegions() -> Promise<region[]>`, `loadUnits(regionId) -> Promise<unitSummary[]>`, `loadUnit(regionId, code) -> Promise<unitDoc>`, `r2Url(region, key)`, `archiveUrl(region, s)`, `detailUrl(region, pole, ext)`, `winner(units, s = 'A') -> unitSummary | null` (lowest regional rank in `s`; `null` summaries sort last; `units[0]` when nobody has a rank), `bboxToBounds(bbox) -> [[south, west], [north, east]]` (the JSON bbox is `[west, south, east, north]`), `pickStart(parsed, visitor, regions, load = loadUnits) -> Promise<{region, unit}>` implementing the visitor fallback order from the Global Constraints.
